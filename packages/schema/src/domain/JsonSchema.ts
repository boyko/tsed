...
import {createRef, serialize} from "../utils/serialize";
...
export class JsonSchema extends Map<string, any> {
  ...
  toJSON(options: IJsonSerializerOptions = {}) {
    const {useAlias = true, schemas = {}, root = true} = options;

    const obj = Array.from(this.entries()).reduce((obj: any, [key, value]) => {
      ...
      if (!root && ["properties", "additionalProperties", "items"].includes(key) && value.isClass) {
        value = createRef(value, {...options, useAlias, schemas, root: false});
      } else {
        value = serialize(value, {...options, useAlias, schemas, root: false});
      }
